...
class DualResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        width_output = x.shape[-1] // 8
        height_output = x.shape[-2] // 8
        # width_output = torch.div(x.shape[-1], 8, rounding_mode='trunc')
        # height_output = torch.div(x.shape[-2], 8, rounding_mode='trunc')
        # H = 1280
        # W = 1280
        # width_output = 160
        # height_output = 160
        ...
        x = self.conv1(x)
        x_0 = self.layer1(x)
        x_1 = self.layer2(self.relu(x_0))
        x_2 = self.layer3(self.relu(x_1))

        x_ = self.layer3_(self.relu(x_1))
        x = x_2 + self.down3(self.relu(x_))
        x_ = x_ + F.interpolate(
            self.compression3(self.relu(x_2)),
            size=[height_output, width_output],
            mode='bilinear')

        x_3 = self.layer4(self.relu(x))
        x_ = self.layer4_(self.relu(x_))

        x = x_3 + self.down4(self.relu(x_))
        x_ = x_ + F.interpolate(
            self.compression4(self.relu(x_3)),
            size=[height_output, width_output],
            mode='bilinear')
...
#         # else:
#         return x_
#         # return (torch.sigmoid(x_) * 255).byte()
#
